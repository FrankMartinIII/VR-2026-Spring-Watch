...
export const init = async model => {
   ...
   if (isHeadset) {
      ...
      // setupPopup creates (or re-creates) the popup and its canvas.
      ...
      try {
	    ...
	    while (shapes.nChildren() > 0)
	       ...
	       if (y < -1.2 || y > .4 || x < -1.2 || x > 1.2)
            ...
            // 2D CLIENT: RECREATE POPUP IF IT WAS CLOSED BY THE USER

            if (!window.popup || window.popup.closed) {
               window.popup = window.open('', 'CanvasWindow', 'width=' + w + ',height=' + h);
               if (window.popup && !popup.canvas) {
                  let canvas = popup.document.createElement('canvas');
                  canvas.width = w;
                  canvas.height = h;
                  canvas.addEventListener('mousedown', e => inputs.mouse = { pressed: true , x: e.x, y: e.y });
                  canvas.addEventListener('mouseup'  , e => inputs.mouse = { pressed: false, x: e.x, y: e.y });
                  canvas.addEventListener('mousemove', e => { inputs.mouse.x = e.x; inputs.mouse.y = e.y; });
                  popup.document.body.style.margin = '0';
                  popup.document.body.appendChild(canvas);
                  popup.canvas = canvas;
                  popup.ctx = canvas.getContext('2d');
	    ...
	    // 2D CLIENT DRAWS THE CURSORS FOR THE TWO VR HANDS
         ...
         console.error('transfer.js animate error:', e);
      }
   });
}
